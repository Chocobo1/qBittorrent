...
#if defined(Q_OS_WIN) || defined(Q_OS_MACOS)
void MainWindow::handleUpdateCheckFinished(const bool invokedByUser, const bool updateAvailable, const QString &newVersion, ProgramUpdater *updater)
{
    ...
    if (updateAvailable)
    {
        ...
        connect(msgBox, &QDialog::finished, this, [this, updater](const int)
        {
            if (m_programUpdateTimer)
                m_programUpdateTimer->start();
            updater->deleteLater();
...
void MainWindow::checkProgramUpdate(const bool invokedByUser)
{
    ...
    connect(updater, &ProgramUpdater::updateCheckFinished
        , this, [this, invokedByUser, updater](const bool updateAvailable, const QString &newVersion)
    {
        handleUpdateCheckFinished(invokedByUser, updateAvailable, newVersion, updater);
    });
