...
{
    const BitTorrent::InfoHash hash {torrent->hash().v1_string()};
...
{
    for (const BitTorrent::TrackerEntry &tracker : trackers)
        m_trackerFilters->addItem(tracker.url(), torrent->hash().v1_string());
}
